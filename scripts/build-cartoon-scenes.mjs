...
import { CARTOON_SOURCE_ITEMS } from "../shared/cartoonCatalog.mjs";
...
const PAINT_CATALOG_PATH = path.resolve(PROJECT_ROOT, "shared", "cartoonPaintCatalog.mjs");
...
async function makeScene(item) {
  ...
  return {
    ...
    paint: dominantPaint(layers.layerData, 4),
  };
}
...
async function main() {
  const outputDirs = Array.from(new Set(CARTOON_SOURCE_ITEMS.map((item) => getOutputDir(item))));
  ...
  const failures = [];
  const results = await mapWithConcurrency(CARTOON_SOURCE_ITEMS, 4, async (item) => {
    try {
  ...
  const paintCatalog = Object.fromEntries(built.map(({ id, paint }) => [id, paint]));
  await fs.writeFile(
    PAINT_CATALOG_PATH,
    `// Generated by scripts/build-cartoon-scenes.mjs from each masked paint layer.\n` +
      ...
      `export const CARTOON_PAINT = ${JSON.stringify(paintCatalog, null, 2)};\n`,
  );
  console.log(`built ${built.length}/${CARTOON_SOURCE_ITEMS.length} real cartoon scenes`);
